...
var TmuxProcess = require('./tmux-process');
var MonitorProcess = require('./monitor-process');
...
function Group(conf) {
  this.conf = conf;
  this.session = conf.session || require('path').basename(process.cwd());
...
Group.prototype.processes = function(){
  ...
  return Object.keys(procs).map(function(key){
    var proc = procs[key];
    if (proc.mode === 'monitor') {
      return new MonitorProcess(self, key, proc);
    }
    return new TmuxProcess(self, key, proc);
  });
};
