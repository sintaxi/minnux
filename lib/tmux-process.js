...
function TmuxProcess(group, name, proc) {
  ...
  this.extensions = [
    path.resolve(join(__dirname, '..', 'extensions', 'stats.ts')),
    path.resolve(join(__dirname, '..', 'extensions', 'windows.ts'))
  ];
...
/**
 * Inject extensions into a pi command.
...
TmuxProcess.prototype.injectExtension = function(cmd){
  var flags = this.extensions.map(function(ext) { return '-e ' + shellQuote(ext); }).join(' ');
  // match "pi" as a standalone command (possibly after && or ;)
  return cmd.replace(/(^|&&\s*|;\s*)pi(\s)/g, '$1pi ' + flags + '$2');
};
