...
module.exports = TmuxProcess;

/**
 * Initialize a `TmuxProcess` of `name` with `cmd`.
 *
 * @param {String} name
 * @param {String} cmd
 * @api public
...
function TmuxProcess(group, name, proc) {
  var conf = group.conf;
  this.mode = 'tmux';
  this.cmd = proc.cmd;
  ...
  // harness for this process (pi, claude, etc)
  this.harnessName = proc.harness || harnesses.defaultHarness;
  this.harness = harnesses.get(this.harnessName);

  // load tmux state — windows are created with sanitized names (dots → dashes)
  var info = group.windowInfo[tmuxName(name)] || group.windowInfo[name];
  if (info) {
    this.pid = info.pid;
...
TmuxProcess.prototype.stats = function(){
  if (this.harness && this.harness.readStats) {
    return this.harness.readStats(this.name, this.baseDir);
  }
  return null;
};
...
TmuxProcess.prototype.mtime = function(){
...
TmuxProcess.prototype.state = function(){
...
TmuxProcess.prototype.alive = function(){
...
TmuxProcess.prototype.target = function(){
  return this.session + ':' + (this.windowName || tmuxName(this.name));
};
...
TmuxProcess.prototype.start = function(fn){
  var self = this;

  // Setup stats tracking for this harness (e.g., Claude statusline)
  // Capture returned options (e.g., addDir for skills)
  if (this.harness && this.harness.setupStats) {
    this.harnessOpts = this.harness.setupStats(this.name, this.baseDir);
  }
  ...
  this.hasSession(function(exists) {
    ...
    exec(tmuxCmd, function(err) {
      ...
      // apply tmux config (always apply to ensure bindings are set)
      applySessionConfig(self.session);
      applyWindowConfig(self.session, winName);

      // pipe pane output to log file
      var target = self.session + ':' + winName;
...
TmuxProcess.prototype.stop = function(sig, fn){
...
TmuxProcess.prototype.hasSession = function(fn){
...
TmuxProcess.prototype.refreshState = function(){
  try {
    var target = this.session + ':' + tmuxName(this.name);
...
TmuxProcess.prototype.wrapCommand = function(){
  var parts = [];
  ...
  // Use harness to build the final command
  var finalCmd = this.cmd;
  if (this.harness && this.harness.buildCommand) {
    finalCmd = this.harness.buildCommand(this.cmd, this.harnessOpts);
  }
...
/**
 * Sanitize a process name for use as a tmux window name.
 * tmux uses '.' as the window/pane separator in target strings,
...
function applySessionConfig(session) {
  ...
  var windowList = '#{W:'
    + '#[range=window|#{window_index}] #W #[norange default],'
    + '#[range=window|#{window_index} list=focus fg=cyan bold] #W #[norange default nobold]'
    + '}';

  var defaults = [
    ['set', '-s', 'extended-keys', 'on'],
    ['set', '-s', 'extended-keys-format', 'csi-u'],
    ['set', '-t', session, 'status', '2'],
    ['set', '-t', session, 'status-position', 'top'],
    ['set', '-t', session, 'status-style', 'bg=default,fg=#888888'],
    ['set', '-t', session, 'status-format[0]', '#[fg=blue,bold] ' + session + ' #[default]'],
    ['set', '-t', session, 'status-format[1]', windowList],
...
function applyWindowConfig(session, name) {
  // window styling is handled inline in status-format[1]
}
